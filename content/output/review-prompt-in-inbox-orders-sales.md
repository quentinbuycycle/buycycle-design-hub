...
## Problem
After a successful transaction, buyers and sellers had no direct prompt to leave a review from the Inbox or Orders & Sales sections, only from within the chat. This limited review completion rates by not surfacing the action in high-traffic areas of the account.

## Solution
A Leave a review row was added to inbox conversation items and order list rows, visible only when booking_status equals paid_out and no review has been submitted yet. Once a review is submitted the row disappears silently, keeping the interface clean.
...
- **Consistency:** Reuses the star and link pattern already established in the mobile app, maintaining cross-platform coherence
